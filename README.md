...
1. **Подготовка файлов:**
   ...
   - Изменяйте CSV-файл (voiceover.csv) с данными для озвучки. В CSV каждая строка должна содержать:
     ...
     - **speaker:** имя говорящего. Если поле пустое, тембр будет определён по текущему аудиосэмплу без кеширования в `workspace/voices`.
...
- **speaker:** имя говорящего, по которому данные группируются для формирования сэмплов. Если поле пустое, тембр берётся напрямую из текущего сэмпла.
...
- Если `speaker` пустой, использует только текущий аудиосэмпл и не создаёт кеш-файл в `workspace/voices`.
